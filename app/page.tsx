"use client";
// TEST: PAGE UPDATED 2026-01-24 00:06
...
export default function Home() {
  ...
  useEffect(() => {
    setChoices((prev) => {
      ...
      // reset disallowed points when #choices changes
      ...
      // auto 3 points for single choice (only if empty)
      if (selectedIds.length === 1) {
        const onlyId = selectedIds[0];
        if (next[onlyId]?.points == null) next[onlyId] = { ...next[onlyId], points: 3 };
      }
  ...
  async function submit() {
    ...
    // points set must match allowed points for count
              ...
              <>
                {selectedLocations.map((l) => {
                  ...
                  return (
                    <div
                      key={l.id}
                      style={{ padding: 12, border: "1px solid #eee", borderRadius: 10, marginBottom: 10 }}
                    >
                ...
                {status.type !== "idle" && (
                  <p style={{ marginTop: 10, color: status.type === "error" ? "#b00020" : "#555" }}>{status.msg}</p>
                )}
              </>
            )}
          </section>
